    ...
    def __init__(self,_buffer=None,UseSerialDebug=False):
        ...
        if not (_buffer is None ):
            ...
            if _buffer.__len__()>=12:
                ...
                #returning the error message from the datasheet insteahd of a number
                if _buffer[5] == 0x10:
...
class FPS_GT511C3(SerialCommander):
    ...
    def Enroll1(self):
        ...
        #Returning the error message instead of a number
        retval = "ACK"
        if not rp.ACK:
            retval = rp.Error
        del rp
    ...
    def GetResponse(self):
        ...
        #Due to the high risk of false-positive (major security issue) it seems better to never let pass a not-full response
        #By this way we are sure no incomplete response could be used by other process
        #By this way we are sure to wait long enough for the incomming response paquet to be complete
        else:
